...
import { deleteJWTCookies, deleteJWTCookiesInMiddleware } from '../_service/cookieHandler';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (refreshToken?.value) {
      ...
      deleteJWTCookies();
      const response = NextResponse.json(getReissueTokenResult);

      deleteJWTCookiesInMiddleware(response);
      return response;
    } else {
      deleteJWTCookies();
      return Response.json({ message: '쿠키에 저장된 리프레시 토큰이 없습니다. ' }, { status: 400 });
    }
  } catch (error) {
    console.error(error);
  }
