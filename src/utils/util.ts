...
export function customLog(message?: unknown): void {
...
export const numberWithCommas = (x: string | number): string => {
  ...
  // 정수부에 콤마를 추가합니다.
  const integerPartWithCommas: string = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ',');
...
export function replaceElementAtIndex<T>(arr: T[], index: number, replacement: T): T[] {
 ...
 * @returns {unknown}
 */
export const downloadFile = async ({ fileName, imageUrl }: { imageUrl: string; fileName: string }) => {
...
export const cutDecimal = (number: number | string, decimalPlaces: number = 1) => {
 ...
 * @returns {*}
 */
export function removeDuplicates<T extends Record<K, unknown>, K extends keyof T>(array: T[], key: K): T[] {
...
export const handleAppLaunch = () => {
  ...
  if (/android/i.test(navigator.userAgent)) {
    ...
    window.open('https://play.google.com/store/apps/details?id=kr.co.kildare.routineon', '_blank');
  } else if (
...
// 쿠키 설정 함수
export function setCookie(res: NextResponse, name: string, value: string, expires: number) {
...
export function shallowCompareNodes(node1: ReactNode, node2: ReactNode): boolean {
  // 둘 다 React 요소인 경우
...
export const makeClass = (className: string, ...arrays: ClassNameValue[]): string => {
...
export function shallowEqual(obj1: unknown, obj2: unknown): boolean {
  ...
  // 객체가 null이나 undefined이거나, 객체가 아닌 경우
  ...
  // 각 키의 값이 같은지 확인
  for (const key of keys1) {
    // obj1과 obj2는 타입스크립트에게 object라고 명시했으므로 'as'를 사용해 타입 단언을 적용
